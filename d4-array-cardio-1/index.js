...
const orderedByYearsLived = inventors.sort(({year: aYear, passed: aPassed}, {year: bYear, passed: bPassed}) => (aPassed - aYear) > (bPassed - bYear) ? -1 : 1);
console.table(orderedByYearsLived);

// https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
// const links = [...document.querySelectorAll('.mw-category-group a')];
// const containsDe = links.map(link => link.textContent).filter(sn => sn.includes('de'))

const alpha = people.sort((a, b) => a.split(',')[0].trim() > b.split(',')[0].trim() ? 1 : -1);
console.table(alpha);
